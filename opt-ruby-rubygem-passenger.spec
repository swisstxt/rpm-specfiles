...
%{?!ruby: %define ruby /opt/ruby-%{rubyver}/bin/ruby}
%{?!rake: %define rake /opt/ruby-%{rubyver}/bin/rake}
%{?!gem:  %define gem  /opt/ruby-%{rubyver}/bin/gem}
...
Requires: opt-ruby-%{rubyver}-rubygem(fastthread) >= 1.0.7
Requires: opt-ruby-%{rubyver}-rubygem(daemon_controller) >= 1.0.0
Requires: opt-ruby-%{rubyver}-rubygem(rack) >= 1.4.1
# Ruby Enterprise doesn't have a devel package
#BuildRequires: ruby-devel
%if !%{only_native_libs}
BuildRequires: httpd-devel
BuildRequires: opt-ruby-%{rubyver}
BuildRequires: opt-ruby-%{rubyver}-rubygem(rack)
BuildRequires: opt-ruby-%{rubyver}-rubygem(fastthread) >= 1.0.1
...
%endif
BuildRequires: opt-ruby-%{rubyver}-rubygem(daemon_controller) >= 1.0.0
BuildRequires: opt-ruby-%{rubyver}-rubygem(file-tail)
# native build deps
%if %{?fedora:1}%{?!fedora:0}
...
#BuildArch: noarch
Provides: opt-ruby-%{rubyver}-rubygem(%{gemname}) = %{passenger_version}
...
Requires: opt-ruby-%{rubyver}-rubygem-passenger-native-libs
Conflicts: rubygem-passenger-native
Conflicts: ree-rubygem-passenger-native
%if %{?fedora:1}%{?!fedora:0}
Requires: libev
%endif
...
This package is compiled agains Ruby Enterprise Edition
...
Requires: opt-ruby-%{rubyver}
Provides: opt-ruby-%{rubyver}-rubygem-passenger-native-libs = %{passenger_epoch}:%{passenger_version}-%{passenger_release}
Conflicts: rubygem-passenger-native-libs
Conflicts: ree-rubygem-passenger-native-libs
...
%package -n opt-ruby-%{rubyver}-passenger-standalone
Summary: Standalone Phusion Passenger Server
Group: System Environment/Daemons
Requires: %{name} = %{passenger_epoch}:%{passenger_version}-%{passenger_release}
Conflicts: ree-passenger-standalone
Conflicts: passenger-standalone
%if %{?fedora:1}%{?!fedora:0}
Requires: libev
%endif
Epoch: %{passenger_epoch}
Obsoletes: opt-ruby-%{rubyver}-rubygem-passenger-standalone
Provides: opt-ruby-%{rubyver}-rubygem-passenger-standalone
%description -n opt-ruby-%{rubyver}-passenger-standalone
Phusion Passenger™ — a.k.a. mod_rails or mod_rack — makes deployment
of Ruby web applications, such as those built on the revolutionary
Ruby on Rails web framework, a breeze. It follows the usual Ruby on
Rails conventions, such as “Don’t-Repeat-Yourself”.
...
%package -n opt-ruby-%{rubyver}-mod_passenger
Summary: Apache Module for Phusion Passenger
Group: System Environment/Daemons
Requires: %{name}-native = %{passenger_epoch}:%{passenger_version}-%{passenger_release}
#BuildArch: %_target_arch
Conflicts: mod_passenger
Conflicts: ree-mod_passenger
Obsoletes: opt-ruby-%{rubyver}-rubygem-passenger-apache
Epoch: %{passenger_epoch}
%description -n opt-ruby-%{rubyver}-mod_passenger
Phusion Passenger™ — a.k.a. mod_rails or mod_rack — makes deployment
of Ruby web applications, such as those built on the revolutionary
Ruby on Rails web framework, a breeze. It follows the usual Ruby on
Rails conventions, such as “Don’t-Repeat-Yourself”.
...
%package -n opt-ruby-%{rubyver}-nginx-passenger
...
%description -n opt-ruby-%{rubyver}-nginx-passenger
Phusion Passenger™ — a.k.a. mod_rails or mod_rack — makes deployment
of Ruby web applications, such as those built on the revolutionary
Ruby on Rails web framework, a breeze. It follows the usual Ruby on
Rails conventions, such as “Don’t-Repeat-Yourself”.
...
# SELINUX
install -p -m 644 -D selinux/rubygem-passenger.pp %{buildroot}%{sharedir}/selinux/packages/%{name}/rubygem-passenger.pp
...
%post -n opt-ruby-%{rubyver}-nginx-passenger
if [ $1 == 1 ]; then
...
%postun -n opt-ruby-%{rubyver}-nginx-passenger
if [ $1 == 0 ]; then
...
semodule -i %{sharedir}/selinux/packages/%{name}/rubygem-passenger.pp 2>/dev/null || :
...
%files -n opt-ruby-%{rubyver}-nginx-passenger
...
%changelog
* Thu Nov 17 2011  mh <mh+rpms@scrit.ch> - 1:3.0.9-1
